...
collector.on('collect', async r => { 
    ...
    // --
        ...
        message.channel.send(`**Options desactivé temporairement.**`)
    } else if(r.emoji.name === '🎁') {
        ...
        if(!channel) return message.channel.send(`**Erreur rencontrée**: veuillez rédefinir le salon du giveaway.`)
       ...
       var timestamp = Date.now() + db.giveaway.duree
        ...
        if (msg.reactions.cache.get("🎉").count <= 1) {
            message.channel(`**Erreur rencontrée**: aucun gagnant`)
        }
