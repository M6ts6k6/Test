...
message.channel.messages.fetch(db.giveaway.last).then(m => {
    if(!m) return message.channel.send(`Le dernier giveaway n'a pas été trouvé, essayez \`&greroll <message id>\``);
    winner = m.reactions.cache.get("🎉").users.cache.filter((u) => !u.bot).random()
    message.channel.send(`Le nouveau gagnant est ${winner}! Félicitations!`)
})
    } else {
