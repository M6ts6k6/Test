...
module.exports.run = async (client, message, args) => {
   ...
   return message.lineReply(`Désolé, mais vous n'avez pas la permission requise pour executer cette commande.`);

      let amount = args[1];

      if (!amount)
        return message.lineReply(`Veuillez spécifier un **nombres** d'invitation à ajouter`);

      let user = message.mentions.users.first();

      if (!user)
      return message.lineReply('Merci de **mentionner** un **utilisateur**');
